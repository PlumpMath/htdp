...
#reader(lib "htdp-beginner-abbr-reader.ss" "lang")((modname htdp-exercise-16-3-4-challenge) (read-case-sensitive #t) (teachpacks ((lib "dir.rkt" "teachpack" "htdp"))) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ((lib "dir.rkt" "teachpack" "htdp")))))
#| 파일
파일은 다음 구조체다.
...
;; find
;; directory, symbol -> list-of-symbols
;; 디렉토리(a-dir)와 파일명(fn)을 입력받아,
;; (find? a-dir fn)의 결과가 true일 경우 : 해당 파일까지의 경로 (디렉토리 이름 리스트)
;; 그렇지 않은 경우 : false
;; 를 출력한다.
(define (find a-dir fn)
  (cond ((find? a-dir fn)
         (append (list (dir-name a-dir))
                 (find-in-lod (dir-dirs a-dir) fn)))
        (else false)))

(define (find-in-lod a-lod fn)
  (cond ((empty? a-lod) empty)
        ((cons? a-lod)
         (cond ((boolean? (find (first a-lod) fn))
                (find-in-lod (rest a-lod) fn))
               (else (find (first a-lod) fn))))))
...
;; 모든 경로 리스트를 출력하는 버전은 16-3-4-challenge-part-2에서 다룸
