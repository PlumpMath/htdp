...
#reader(lib "htdp-beginner-abbr-reader.ss" "lang")((modname htdp-exercise-14-3-4) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ())))
...
(define (depth a-wp)
  (cond ((empty? a-wp) 0)
        ((cons? a-wp)
         (cond ((symbol? (first a-wp)) (depth (rest a-wp)))
               ((cons? (first a-wp))
                (cond ((< (+ 1 (depth (first a-wp))) (depth (rest a-wp))) (depth (rest a-wp)))
                      (else (+ 1 (depth (first a-wp))))))
               (else (error 'depth "invalid WP"))))
        (else (error 'depth "invalid WP"))))

;; examples
(depth empty)
(depth (list 'a 'b 'c 'd 'a 'b 'c))
(depth (list (list 'a 'b 'c 'd)))
(depth (list 'a 'b (list 'a 'b 'c 'd) 'c 'd))
(depth (list (list (list 'a) 'b (list 'a 'b) 'c 'd)))
